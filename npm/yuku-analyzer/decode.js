...
const IMPORT_PHASES = ["source", "defer", "lazy"];
...
function decode(buffer, source) {
  ...
  function _decode(i) {
    ...
    case 3: {
      ...
      if (f0) r.decorators = nodeArr(f4, f0);
      if (_isTs) {
        r.typeParameters = f4 !== NULL ? node(f5) : null;
        r.returnType = f5 !== NULL ? node(f6) : null;
        r.declare = !!(flags & 16);
      }
      return r;
    }
  ...
  function _semantic() {
    ...
    return (_semView = {
      ...
      import: {
        ...
        phase: (i) =>
          (imports[i * 8 + 1] >> 4) & 1
            ? IMPORT_PHASES[(imports[i * 8 + 1] >> 5) & 3]
            : null,
